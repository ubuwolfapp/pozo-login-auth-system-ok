  ...
  return <div className="min-h-screen bg-slate-900 text-white pb-20">
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="bg-slate-800 p-4 rounded-lg mb-6 space-y-4">
          ...
          <Input placeholder="Buscar por título, descripción o asignado..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="bg-slate-700 border-slate-600 text-white" />
          ...
          <div className="space-y-4">
            {filteredTasks.length > 0 ? filteredTasks.map(task => <Card key={task.id} className="bg-slate-800 border-slate-700 p-4">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-slate-50">Fecha: {format(new Date(task.created_at), 'dd/MM/yyyy HH:mm')}</p>
                    {task.fecha_limite && <p className="text-red-600">Límite: {format(new Date(task.fecha_limite), 'dd/MM/yyyy')}</p>}
                  </div>
